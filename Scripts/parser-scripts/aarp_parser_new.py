...
t0 = time.time()
AARP_HOST = "http://healthtools.aarp.org"
...
def createJsonFile(outputObj):
	with open("../data/diseases_symptoms_aarp_new.json", "w") as outfile:
...
def getDiseaseDescription(diseaseURL):
	description = ""
	htmlContainer = makeRequest(diseaseURL, "div", "id", "TOC_TITLE_1")
	#print htmlContainer
	if(len(htmlContainer) > 0):
		description = str(htmlContainer[0].get_text().encode('utf-8')).replace("\n","")
...
def processDiseases(tableContainer):
	...
	for child in tableContainer[0].children:
		...
		if(len(childStr) != 0):
			...
			spanTag = soupObj.find("span", {"class" : "articletitle"})
			# print "----------------------------------------------------"
			relativePath = spanTag.find("a", {"class": "a-black-darkred"})["href"]
			diseaseURL = AARP_HOST + relativePath
			# print diseaseURL
			description = getDiseaseDescription(diseaseURL)
			# print description
			# print len(description)
			if(description == ""):
				description = str(spanTag.next_sibling.get_text().encode('utf-8')).replace("\n","")
			# print "----------------------------------------------------"
			diseaseName = spanTag.get_text().encode('utf-8')
			...
			DISEASES_DESCRIPTION_DICT[diseaseName] = description
	#print DISEASES_DICT
	return
...
print len(DISEASES_DESCRIPTION_DICT)
